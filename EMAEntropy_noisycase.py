# draw the EMAE distribution graph of clean samples and noisy samples after warm-up training
# only need to run warm-up epochs
...
parser.add_argument('--recording_file', default='validate_EMA_noisy_try4', help='name of recording file, validate_EMA_noisy_try1')
...
parser.add_argument('--update_times', type=int, default=0, help='times of updating selected training dataset')
parser.add_argument('--n_epoch', type=int, default=100, help='each time training epoch')
...
parser.add_argument('--threshold_start', type=float, default=0.5, help='try to adopt a linear time-changing threshold')
...
# noise
parser.add_argument('--noise_type', type=str, default='symmetric', help='pairflip, symmetric, instance')
...
# ============ get noisy data ============= # we put it outside because we need to analyse results and the dataset should be kept the same
...
def main():
    ...
    # 在这里加一步把EMAE的dict保存下来就可以了
    tf = open(os.path.join(log_dir, "EMAEntropy_dict.pkl"), "wb")
    pickle.dump(emae_best_dict, tf)
    tf.close()
...
def Initialize(emae_dict, pred_dict, recording_file, recording_file_0, train_loader):
    for i, data in enumerate(train_loader):
        ...
        for j in range(len(targets)):
            ...
            emae_dict[path] = None
            pred_dict[path] = None
            # 这两行不能去掉 因为要用这个来记录哪些数据是clean和noisy的
...
def validate_EMAE(recording_file):
    ''' we only analyze the EMAE after warm-up training
    '''
    BASE_DIR = os.path.dirname(os.path.abspath(__file__))
    log_dir = os.path.join(BASE_DIR, "results", recording_file)
    data_dir = os.path.join(BASE_DIR ,'./public_data/cifar10')
    EMAEntropy_dict = pickle.load(open(os.path.join(log_dir, 'EMAEntropy_dict.pkl'), 'rb'))
    df = pd.read_csv(os.path.join(log_dir, "Recording.csv"))
    # extract clean data's paths and noisy data's paths
    clean_paths =list(df[df['true_label']==df['noisy_label']]['path'])
    noisy_paths =list(df[df['true_label']!=df['noisy_label']]['path'])
    # draw distributions of EMAE
    # overall
    entropys = list(EMAEntropy_dict.values())
    plt.subplot(2,1,1)
    plt.hist(entropys, bins=80)
    plt.title("EMAE overall")
    # clean
    entropys_clean = list(itemgetter(*clean_paths)(EMAEntropy_dict))
    # plt.subplot(2, 2, 2)
    # plt.hist(entropys_clean, bins=80)
    # plt.title("EMAE clean")
    # # noisy
    entropys_noisy = list(itemgetter(*noisy_paths)(EMAEntropy_dict))
    # plt.subplot(2,2,3)
    # plt.hist(entropys_noisy, bins = 80)
    # plt.title("EMAE noisy")
    # interact
    plt.subplot(2,1,2)
    plt.hist(entropys_clean, bins = 80, alpha =0.5, label='clean')
    plt.hist(entropys_noisy, bins = 80, alpha=0.5, label='noisy')
    plt.legend(loc='upper right')
    plt.title('EMAE interact')

    plt.savefig(os.path.join(log_dir, 'distributions.jpg'))

    # draw the smallest100 and largest100 EMAE figures
    train_dataset = Train_Dataset(train_data, train_noisy_labels, train_clean_labels, train_paths, transform=transforms.Compose([transforms.ToTensor()]))
    train_loader = DataLoader(dataset=train_dataset, batch_size=args.batch_size, num_workers=8, shuffle=True, pin_memory=True)
    ...
    EMAE_small100_paths= EMAE_sorted_indexes[:100]
    EMAE_large100_paths = EMAE_sorted_indexes[-100:]
    ...
    for i, data in enumerate(train_loader):
        (imgs, targets, clean_labels, paths) = data
        for j in range(len(paths)):
            if paths[j] in EMAE_small100_paths:
                count+=1
                plt.subplot(10, 10, count)
                plt.imshow(imgs[j].numpy().transpose(1, 2, 0))
                if targets[j].item()!=clean_labels[j].item():
                    plt.title(str(classes[clean_labels[j].item()]), fontdict={'fontsize': 6}, color='red')
                else:
                    plt.title(str(classes[clean_labels[j].item()]), fontdict={'fontsize': 6})
                plt.xticks([])
                plt.yticks([])
    plt.savefig(os.path.join(log_dir, 'EMAEntropy_small100.jpg'))
    ...
    for i, data in enumerate(train_loader):
        (imgs, targets, clean_labels, paths) = data
        for j in range(len(paths)):
            if paths[j] in EMAE_large100_paths:
                count += 1
                plt.subplot(10, 10, count)
                plt.imshow(imgs[j].numpy().transpose(1, 2, 0))
                if targets[j].item() != clean_labels[j].item():
                ...
                plt.xticks([])
                plt.yticks([])
    plt.savefig(os.path.join(log_dir, 'EMAEntropy_large100.jpg'))


if __name__ == '__main__':
    validate_EMAE('validate_EMA_noisy_try4')
    # main()

# use MWN to train imbalanced datasets

import os
import time
import argparse
from torch.utils.data.sampler import SubsetRandomSampler
from datasets import build_dataset, cal_num_per_cls, build_imbalanced_train_dataset
from resnet_MWN import *
...
parser.add_argument('--recording_file', default='MWN_imbalance', help='name of recording file')
...
# get imbalanced dataset and dataloader
...
num_per_cls = cal_num_per_cls(args.dataset,args.imb_factor,args.num_meta*args.num_classes)
imbalanced_train_dataset = build_imbalanced_train_dataset(train_data, num_per_cls)
...
def main_class_imbalance():
    ...
    criterion = nn.CrossEntropyLoss().cuda()  # 用的CE loss

    for epoch in range(args.epochs):
        adjust_learning_rate(optimizer_a, epoch + 1) # 调整learning rate

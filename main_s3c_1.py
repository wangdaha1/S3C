...
# adapt: adjust cushion on warm-up training (strengthen the first early stop), and decrease n_epoch
# adapt: eliminate the second early stop, and decrease the n_epoch in alternative training (may be still the same as the warm-up epochs)
...
parser.add_argument('--cushion', type=int, default=10, help='cushion epochs in warm-up training')
# noise
...
def main():
    ...
    for epoch in range(args.n_epoch): # may be different from later training epochs
        ...
        if (acc_avg_eval > inner_val_acc + n_add):
            ...
            n_add += 0.001
        if n_add > (0.001*args.cushion): # cushion parameter
    ...
    for update_time in range(args.update_times):
        ...
        for epoch in range(args.n_epoch):
            ...
            loss_avg_eval, acc_avg_eval = evaluate(model, val_loader, criterion, device)
            scheduler.step()
            inner_val_acc=acc_avg_eval # 以最后一轮的val结果作为这一轮最好的结果就好了 看之前的结果差不多的额
            # loss_rec['train'].append(loss_avg_train); loss_rec['valid'].append(loss_avg_eval)
            # acc_rec['train'].append(acc_avg_train); acc_rec['valid'].append(acc_avg_eval)
            # if (epoch + 1) % 5 == 0:
                # draw_line(loss_rec['train'], loss_rec['valid'], log_dir, 'loss')
        ...
        train_state_epoch[update_time + 1] = args.n_epoch
        torch.save(model.state_dict(), model_save_path)
